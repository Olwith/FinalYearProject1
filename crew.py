...
def send_notification_to_customer(customer_id, message):
    ...
    try:
        cursor.execute("""
            INSERT INTO Notification (user_id, message, status, created_at)
            VALUES (%s, %s, 'unread', NOW())
        """, (customer_id, message))
        conn.commit()
    except Exception as e:
...
def resolve_task(outage_id):
    ...
    try:
        ...
        # 1. Get customer ID before updating
        cursor.execute("""
            SELECT customer_id 
            FROM Outage 
            WHERE id = %s
        ...
        cursor.execute("""
            ...
            WHERE id = (SELECT assigned_crew_id FROM Outage WHERE id = %s)
        """, (outage_id,))
...
def get_route(start_lat, start_lon, end_lat, end_lon):
    ...
    try:
        ...
        eta = distance * 2  # Estimate 2 minutes per km
...
if st.session_state.active_tab == 'dashboard':
    ...
    if st.session_state.crew_lat and st.session_state.crew_lon:
        m = folium.Map(
            location=[st.session_state.crew_lat, st.session_state.crew_lon],
            zoom_start=12,
            width='100%',
            tiles='OpenStreetMap'
        )
        ...
        for incident in nearby_incidents:
            ...
            if assigned_crew_id:
                if assigned_crew_id == st.session_state.crew_id:
                    ...
                    route_coords, route_distance, route_eta = get_route(st.session_state.crew_lat, st.session_state.crew_lon, lat, lon)
                    if route_coords:
                        folium.PolyLine(
                            route_coords,
                            color="green",
